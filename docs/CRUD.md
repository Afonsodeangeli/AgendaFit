...
## ⚠️ IMPORTANTE: CRUD de Categoria Foi Removido

### O que Aconteceu?

O CRUD completo de **Categoria** foi **removido intencionalmente** deste projeto para fins educacionais.

Você deverá **recriá-lo do zero** seguindo este tutorial, aplicando todos os conceitos e padrões aprendidos.
...
**1. main.py**
- Removido import de `categoria_repo`
- Removida criação da tabela `categoria`

**2. model/atividade_model.py**
- Removido import de `Categoria`
- Removido campo `id_categoria`
- Removido campo `categoria: Optional[Categoria]`

**3. sql/atividade_sql.py**
- Removida coluna `id_categoria` da tabela
- Removida FOREIGN KEY para `categoria`
- Removidos JOINs com tabela `categoria`
- Removida query `OBTER_POR_CATEGORIA`

**4. repo/atividade_repo.py**
- Removido import de `Categoria`
- Removida função `obter_por_categoria()`
- Removida construção de objetos `Categoria` nas queries

**5. dtos/atividade_dto.py**
- Removido campo `id_categoria` de `CriarAtividadeDTO`
- Removido campo `id_categoria` de `AlterarAtividadeDTO`
- Removidos validadores de `id_categoria`

**6. routes/admin_atividades_routes.py**
- Removido import de `categoria_repo`
- Removidas validações de categoria
- Removidos parâmetros `id_categoria` dos formulários
- Removida verificação de existência de categorias

**7. templates/admin/atividades/listar.html**
- Removida coluna "Categoria" da tabela

**8. templates/admin/atividades/cadastrar.html**
- Removido campo select de categoria

**9. templates/admin/atividades/editar.html**
- Removido campo select de categoria

**10. templates/dashboard.html**
- Removido card de "Categorias"

**11. Banco de Dados (dados.db)**
- **Tabela `categoria` foi DELETADA**
- **Coluna `id_categoria` foi REMOVIDA da tabela `atividade`**
...
Execute a aplicação para confirmar que o CRUD de categoria foi removido:
...
- ✅ O menu não tem link para "Categorias"
- ✅ O dashboard não mostra card de "Categorias"
- ✅ Atividades podem ser criadas SEM categoria

#### Passo 2: Entender a Tabela do Banco de Dados

A tabela `atividade` agora tem esta estrutura simplificada:
...
**Nota**: Não há mais `id_categoria` nem Foreign Key!

Para verificar:

```bash
sqlite3 dados.db "PRAGMA table_info(atividade);"
```

#### Passo 3: Recriar o CRUD de Categoria

Agora você deve seguir **todos os passos deste tutorial** para recriar o CRUD de categoria:

1. ✅ **Passo 1**: Criar `sql/categoria_sql.py` com as queries SQL
2. ✅ **Passo 2**: Criar `model/categoria_model.py` com o dataclass
3. ✅ **Passo 3**: Criar `dtos/categoria_dto.py` com validações Pydantic
4. ✅ **Passo 4**: Criar `repo/categoria_repo.py` com operações CRUD
5. ✅ **Passo 5**: Registrar `categoria_repo` no `main.py`
6. ✅ **Passo 6**: Criar `routes/admin_categorias_routes.py` com endpoints
7. ✅ **Passo 7**: Registrar rotas no `main.py`
8. ✅ **Passo 8**: Criar template `templates/admin/categorias/listar.html`
9. ✅ **Passo 9**: Criar template `templates/admin/categorias/cadastrar.html`
10. ✅ **Passo 10**: Criar template `templates/admin/categorias/editar.html`
11. ✅ **Passo 11**: Adicionar link no menu (se aplicável)
12. ✅ **Passo 12**: Adicionar card no dashboard
13. ✅ **Passo 13**: Testar o CRUD completo

#### Passo 4: Reconectar Categoria com Atividade

Após criar o CRUD básico de categoria, você precisará **reconectar** a entidade Categoria com Atividade. Este passo é OPCIONAL e deve ser feito APENAS após o CRUD de categoria estar funcionando.

Consulte o tutorial completo nas seções anteriores para ver os exemplos detalhados de como:
- Adicionar coluna `id_categoria` na tabela `atividade`
- Adicionar Foreign Key
- Atualizar models, DTOs, repositories e routes
- Atualizar templates para mostrar e selecionar categoria
...
#### Erro: "no such table: categoria"

**Causa**: A tabela `categoria` foi deletada do banco.

**Solução**: Siga o Passo 3 para recriar o CRUD de categoria, incluindo a criação da tabela.

#### Erro: "no such column: atividade.id_categoria"

**Causa**: A coluna `id_categoria` foi removida da tabela `atividade`.

**Solução**: Siga o Passo 4 para reconectar categoria com atividade e migrar o banco de dados.

#### Erro: "ImportError: cannot import name 'Categoria'"

**Causa**: O arquivo `model/categoria_model.py` não existe ou não está sendo importado corretamente.

**Solução**:
1. Certifique-se de que você criou o arquivo `model/categoria_model.py`
2. Verifique se o import está correto: `from model.categoria_model import Categoria`
3. Limpe o cache Python: `find . -name "__pycache__" -type d -exec rm -rf {} +`

#### Erro: "No module named 'categoria_repo'"

**Causa**: O arquivo `repo/categoria_repo.py` não existe.

**Solução**: Siga o Passo 4 do tutorial para criar o repositório.

---

### Checklist Final

Antes de considerar o CRUD completo, verifique:

- [ ] Tabela `categoria` criada no banco de dados
- [ ] Model `Categoria` funcionando corretamente
- [ ] DTOs de categoria validando campos
- [ ] Repository de categoria com todas as operações CRUD
- [ ] Rotas de categoria registradas e acessíveis
- [ ] Templates de categoria renderizando corretamente
- [ ] Dashboard mostra card de "Categorias"
- [ ] Testes básicos funcionando

**OPCIONAL** (reconexão com atividade):
- [ ] Tabela `atividade` tem coluna `id_categoria` e FOREIGN KEY
- [ ] Atividades só podem ser criadas após selecionar uma categoria
- [ ] Listagem de atividades mostra nome da categoria
...
Esta remoção foi feita propositalmente para que você pratique a **implementação completa** de um CRUD seguindo os padrões do projeto.

**Ao terminar, você terá:**
- ✅ Dominado a arquitetura em camadas
- ✅ Praticado todos os conceitos do tutorial
- ✅ Criado um CRUD funcional e integrado
- ✅ Entendido as relações entre entidades
