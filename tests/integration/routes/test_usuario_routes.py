...
class TestDashboard:
    ...
    def test_dashboard_usuario_aluno(self, aluno_autenticado):
        """Aluno autenticado pode acessar dashboard"""
        response = aluno_autenticado.get("/usuario")
        assert response.status_code == status.HTTP_200_OK

    def test_dashboard_exibe_dados_usuario(self, aluno_autenticado, usuario_teste):
    ...
    def test_dashboard_professor(self, professor_autenticado, professor_teste):
        """Professor autenticado pode acessar dashboard"""
        response = professor_autenticado.get("/usuario")
        assert response.status_code == status.HTTP_200_OK
        assert professor_teste["nome"] in response.text
...
class TestVisualizarPerfil:
    ...
    def test_visualizar_perfil_usuario_autenticado(
        self, aluno_autenticado, usuario_teste
    ):
        """Usuário autenticado deve ver seu perfil"""
        response = aluno_autenticado.get("/usuario/perfil/visualizar")
        assert response.status_code == status.HTTP_200_OK
        assert usuario_teste["nome"] in response.text
        assert usuario_teste["email"] in response.text

    def test_visualizar_perfil_usuario_nao_encontrado(self, aluno_autenticado):
        ...
        with patch("routes.usuario_routes.usuario_repo") as mock_repo:
            ...
            response = aluno_autenticado.get(
                "/usuario/perfil/visualizar", follow_redirects=False
            )
...
class TestEditarPerfil:
    ...
    def test_get_formulario_edicao_usuario_autenticado(self, aluno_autenticado):
        """Usuário autenticado deve acessar formulário de edição"""
        response = aluno_autenticado.get("/usuario/perfil/editar")
        assert response.status_code == status.HTTP_200_OK
        assert "editar" in response.text.lower() or "perfil" in response.text.lower()

    def test_editar_perfil_com_dados_validos(self, aluno_autenticado, usuario_teste):
        """Deve permitir editar perfil com dados válidos"""
        response = aluno_autenticado.post(
            "/usuario/perfil/editar",
            data={"nome": "Nome Atualizado", "email": usuario_teste["email"]},
        ...
        response_visualizar = aluno_autenticado.get("/usuario/perfil/visualizar")
        assert "Nome Atualizado" in response_visualizar.text

    def test_editar_perfil_com_email_duplicado(
        self, client, criar_usuario, usuario_teste
    ):
        """Deve rejeitar edição com email já usado por outro usuário"""
        criar_usuario(
            usuario_teste["nome"], usuario_teste["email"], usuario_teste["senha"]
        )
        criar_usuario("Outro Usuario", "outro@example.com", "Senha@123")

        # Login como segundo usuário
        client.post("/login", data={"email": "outro@example.com", "senha": "Senha@123"})

        # Tentar alterar email para o do primeiro usuário
        response = client.post(
            "/usuario/perfil/editar",
            data={"nome": "Outro Usuario", "email": usuario_teste["email"]},
    ...
    def test_editar_perfil_com_nome_vazio(self, aluno_autenticado, usuario_teste):
        """Deve rejeitar nome vazio"""
        response = aluno_autenticado.post(
            "/usuario/perfil/editar",
            data={"nome": "", "email": usuario_teste["email"]},
            follow_redirects=True,
        )
    ...
    def test_editar_perfil_com_email_invalido(self, aluno_autenticado):
        """Deve rejeitar email inválido"""
        response = aluno_autenticado.post(
            "/usuario/perfil/editar",
            data={"nome": "Nome Válido", "email": "email-invalido"},
            follow_redirects=True,
        )
    ...
    def test_editar_perfil_atualiza_sessao(self, aluno_autenticado, usuario_teste):
        """Editar perfil deve atualizar dados na sessão"""
        aluno_autenticado.post(
            "/usuario/perfil/editar",
            data={"nome": "Nome Atualizado", "email": "novoemail@example.com"},
        )

        response = aluno_autenticado.get("/usuario")
        assert "Nome Atualizado" in response.text

    def test_get_editar_perfil_rate_limit(self, aluno_autenticado):
        """Rate limit deve bloquear acesso ao formulário"""
        with patch(
            "routes.usuario_routes.form_get_limiter.verificar", return_value=False
        ):
            response = aluno_autenticado.get(
                "/usuario/perfil/editar", follow_redirects=False
            )
...
class TestAlterarSenha:
    ...
    def test_get_formulario_alterar_senha_usuario_autenticado(self, aluno_autenticado):
        """Usuário autenticado deve acessar formulário de alteração de senha"""
        response = aluno_autenticado.get("/usuario/perfil/alterar-senha")
        assert response.status_code == status.HTTP_200_OK
        assert_contains_text(response, "senha")

    def test_alterar_senha_com_dados_validos(self, aluno_autenticado, usuario_teste):
        ...
        aluno_autenticado.get("/logout")
        response_login = aluno_autenticado.post(
            "/login",
            data={"email": usuario_teste["email"], "senha": "NovaSenha@123"},
            follow_redirects=False,
        )

        assert response_login.status_code == status.HTTP_303_SEE_OTHER

    def test_alterar_senha_com_senha_atual_incorreta(self, aluno_autenticado):
    ...
    def test_alterar_senha_nova_igual_atual(self, aluno_autenticado, usuario_teste):
    ...
    def test_alterar_senha_senhas_nao_coincidem(self, aluno_autenticado, usuario_teste):
    ...
    def test_alterar_senha_nova_fraca(self, aluno_autenticado, usuario_teste):
    ...
    def test_get_alterar_senha_rate_limit(self, aluno_autenticado):
        """Rate limit deve bloquear acesso ao formulário"""
        with patch(
            "routes.usuario_routes.form_get_limiter.verificar", return_value=False
        ):
            response = aluno_autenticado.get(
                "/usuario/perfil/alterar-senha", follow_redirects=False
            )

            assert response.status_code == status.HTTP_303_SEE_OTHER
            assert response.headers["location"] == "/usuario"

    def test_post_alterar_senha_rate_limit(self, aluno_autenticado):
        """Rate limit deve bloquear alteração de senha"""
        with patch(
            "routes.usuario_routes.alterar_senha_limiter.verificar", return_value=False
        ):
            response = aluno_autenticado.post(
                "/usuario/perfil/alterar-senha",
                data={
...
class TestAtualizarFoto:
    ...
    def test_atualizar_foto_requer_autenticacao(self, client, foto_teste_base64):
        ...
        response = client.post(
            ...
            data={"foto_base64": foto_teste_base64},
            follow_redirects=False,
        )
        assert_permission_denied(response)

    def test_atualizar_foto_com_dados_validos(
        self, aluno_autenticado, foto_teste_base64
    ):
    ...
    def test_atualizar_foto_com_dados_invalidos(self, aluno_autenticado):
        """Deve rejeitar dados inválidos"""
        response = aluno_autenticado.post(
            "/usuario/perfil/atualizar-foto",
            data={"foto_base64": "dados-invalidos"},
            follow_redirects=False,
        )

        assert response.status_code == status.HTTP_303_SEE_OTHER

    def test_atualizar_foto_muito_grande(self, aluno_autenticado):
    ...
    def test_atualizar_foto_vazia(self, aluno_autenticado):
    ...
    def test_post_atualizar_foto_rate_limit(self, aluno_autenticado):
        """Rate limit deve bloquear upload de foto"""
        with patch(
            "routes.usuario_routes.upload_foto_limiter.verificar", return_value=False
        ):
            response = aluno_autenticado.post(
                "/usuario/perfil/atualizar-foto",
    ...
    def test_atualizar_foto_erro_io(self, aluno_autenticado):
        """Deve tratar erro de I/O ao salvar foto"""
        with patch(
            "routes.usuario_routes.salvar_foto_cropada_usuario",
    ...
    def test_atualizar_foto_erro_os(self, aluno_autenticado):
