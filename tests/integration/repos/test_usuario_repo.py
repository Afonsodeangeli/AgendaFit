...
class TestUsuarioRepoInserir:
    ...
    def test_inserir_usuario_com_perfil_professor(self):
        """Deve inserir usuário professor corretamente."""
        usuario = Usuario(
            id=0,
        ...
        assert usuario_salvo.perfil == Perfil.PROFESSOR.value
...
class TestUsuarioRepoAlterar:
    ...
    def test_alterar_perfil_usuario(self):
        ...
        usuario.id = usuario_id
        usuario.perfil = Perfil.PROFESSOR.value
        resultado = usuario_repo.alterar(usuario)

        assert resultado is True
        usuario_alterado = usuario_repo.obter_por_id(usuario_id)
        assert usuario_alterado.perfil == Perfil.PROFESSOR.value
...
class TestUsuarioRepoObterTodosPorPerfil:
    ...
    def test_obter_todos_por_perfil_aluno(self):
        """Deve retornar apenas usuários com perfil Aluno."""
        ...
        professor = Usuario(
            ...
            email="professor_perfil@example.com",
            senha=criar_hash_senha("Senha@123"),
            perfil=Perfil.PROFESSOR.value
        )
        ...
        resultado = usuario_repo.obter_todos_por_perfil(Perfil.ALUNO.value)

        assert isinstance(resultado, list)
        for usuario in resultado:
            assert usuario.perfil == Perfil.ALUNO.value

    def test_obter_todos_por_perfil_professor(self):
        """Deve retornar apenas usuários com perfil Professor."""
        professor = Usuario(
            id=0,
            nome="Professor Filtro",
            email="professor_filtro@example.com",
            senha=criar_hash_senha("Senha@123"),
            perfil=Perfil.PROFESSOR.value
        )
        usuario_repo.inserir(professor)

        resultado = usuario_repo.obter_todos_por_perfil(Perfil.PROFESSOR.value)
