...
class TestLogin:
    ...
    def test_login_com_email_invalido(self, client):
        ...
        response = client.post(
            "/login",
            data={"email": "naoexiste@example.com", "senha": "SenhaQualquer@123"},
    ...
    def test_login_com_senha_incorreta(self, client, criar_usuario, usuario_teste):
        ...
        response = client.post(
            "/login",
            data={"email": usuario_teste["email"], "senha": "SenhaErrada@123"},
            follow_redirects=True,
        )
    ...
    def test_login_com_email_vazio(self, client):
        ...
        response = client.post(
            "/login", data={"email": "", "senha": "Senha@123"}, follow_redirects=True
        )

        assert response.status_code == status.HTTP_200_OK
        texto = response.text.lower()
        assert (
            "string_too_short" in texto or "obrigatório" in texto or "e-mail" in texto
        )

    def test_usuario_logado_nao_acessa_login(self, aluno_autenticado):
        """Usuário já logado deve ser redirecionado ao acessar /login"""
        response = aluno_autenticado.get("/login", follow_redirects=False)
        assert_redirects_to(response, "/usuario")
...
class TestCadastro:
    ...
    def test_cadastro_cria_usuario_com_perfil_aluno(self, client):
        """Cadastro público deve criar usuário com perfil ALUNO (Enum Perfil)"""
        ...
        assert usuario.perfil == Perfil.ALUNO.value  # Usa Enum Perfil


class TestLogout:
    """Testes de logout"""

    def test_logout_limpa_sessao(self, aluno_autenticado):
        """Logout deve limpar sessão e redirecionar para raiz"""
        response = aluno_autenticado.get("/logout", follow_redirects=False)

        # Deve redirecionar para raiz do site
        assert_redirects_to(response, "/")

    def test_logout_desautentica_usuario(self, aluno_autenticado):
        """Após logout, usuário não deve ter acesso a áreas protegidas"""
        # Fazer logout
        aluno_autenticado.get("/logout")

        # Tentar acessar área protegida
        response = aluno_autenticado.get("/chamados/listar", follow_redirects=False)
...
class TestRecuperacaoSenha:
    ...
    def test_solicitar_recuperacao_senha_email_existente(
    ...
    def test_redefinir_senha_com_token_invalido(self, client):
        ...
        response = client.get(
            "/redefinir-senha?token=token_invalido", follow_redirects=False
        )

        # Deve redirecionar
        assert response.status_code == status.HTTP_303_SEE_OTHER

    def test_redefinir_senha_com_dados_validos(
        self, client, criar_usuario, usuario_teste
    ):
...
class TestAutorizacao:
    ...
    def test_usuario_autenticado_acessa_area_protegida(self, aluno_autenticado):
        """Usuário autenticado deve acessar áreas protegidas"""
        response = aluno_autenticado.get("/chamados/listar")
        assert response.status_code == status.HTTP_200_OK

    def test_aluno_nao_acessa_area_admin(self, aluno_autenticado):
        """Aluno não deve acessar áreas administrativas"""
...
class TestRateLimiting:
    ...
    def test_multiplas_tentativas_login_falhas_bloqueiam_ip(self, client):
        ...
        for i in range(6):  # Mais que o limite de 5
            response = client.post(
                "/login",
                data={"email": "teste@example.com", "senha": "SenhaErrada@123"},
                follow_redirects=True,
            )
...
class TestValidarUrlRedirect:
    ...
    def test_url_protocolo_relativo_bloqueia(
        self, client, criar_usuario, usuario_teste
    ):
...
class TestEsqueciSenhaRateLimit:
    ...
    def test_multiplas_solicitacoes_bloqueiam(self, client):
        ...
        response2 = client.post(
            "/esqueci-senha", data={"email": "teste@example.com"}, follow_redirects=True
        )
...
class TestCadastroAdicional:
    ...
    def test_usuario_logado_nao_acessa_cadastro(self, aluno_autenticado):
        """Usuário já logado deve ser redirecionado ao acessar /cadastrar"""
        response = aluno_autenticado.get("/cadastrar", follow_redirects=False)
        assert_redirects_to(response, "/usuario")
    ...
    def test_cadastro_erro_ao_inserir(self, client):
        ...
        with patch(
            "routes.auth_routes.verificar_email_disponivel", return_value=(True, None)
        ):
...
class TestEsqueciSenhaAdicional:
    ...
    def test_email_invalido_validation_error(self, client):
        ...
        response = client.post("/esqueci-senha", data={"email": "email-invalido"})
    ...
    def test_email_enviado_com_sucesso(self, client, criar_usuario, usuario_teste):
        ...
        with patch(
            "routes.auth_routes.servico_email.enviar_recuperacao_senha",
            return_value=True,
        ):
            response = client.post(
                "/esqueci-senha",
...
class TestRedefinirSenhaAdicional:
    ...
    def test_get_redefinir_senha_token_valido(
        self, client, criar_usuario, usuario_teste
    ):
    ...
    def test_get_redefinir_senha_token_expirado(
        self, client, criar_usuario, usuario_teste
    ):
    ...
    def test_post_redefinir_senha_token_expirado(
        self, client, criar_usuario, usuario_teste
    ):
    ...
    def test_post_redefinir_senha_sucesso_completo(
        self, client, criar_usuario, usuario_teste
    ):
    ...
    def test_post_redefinir_senha_validation_error(
        self, client, criar_usuario, usuario_teste
    ):
