...
class TestTema:
    ...
    def test_get_tema_requer_admin(self, aluno_autenticado):
        """Aluno não deve acessar seletor de temas"""
        response = aluno_autenticado.get("/admin/tema", follow_redirects=False)
    ...
    def test_aplicar_tema_existente(self, admin_autenticado):
        ...
        if css_original.exists():
            response = admin_autenticado.post(
                "/admin/tema/aplicar", data={"tema": "original"}, follow_redirects=False
            )
    ...
    def test_aplicar_tema_inexistente(self, admin_autenticado):
        ...
        response = admin_autenticado.post(
            "/admin/tema/aplicar",
            data={"tema": "tema_que_nao_existe_xyz123"},
    ...
    def test_aplicar_tema_limpa_cache(self, admin_autenticado):
        ...
        if css_original.exists():
            ...
            # Aplicar tema
            admin_autenticado.post("/admin/tema/aplicar", data={"tema": "original"})
    ...
    def test_aluno_nao_pode_aplicar_tema(self, aluno_autenticado):
        """Aluno não deve poder aplicar tema"""
...
class TestAuditoria:
    ...
    def test_get_auditoria_requer_admin(self, aluno_autenticado):
        """Aluno não deve acessar auditoria"""
        response = aluno_autenticado.get("/admin/auditoria", follow_redirects=False)
        assert response.status_code in [
            status.HTTP_303_SEE_OTHER,
            status.HTTP_403_FORBIDDEN,
        ]
    ...
    def test_filtrar_logs_por_data(self, admin_autenticado):
        ...
        response = admin_autenticado.post(
            "/admin/auditoria/filtrar", data={"data": data_hoje, "nivel": "TODOS"}
        )
    ...
    def test_filtrar_logs_nivel_info(self, admin_autenticado):
        ...
        response = admin_autenticado.post(
            "/admin/auditoria/filtrar", data={"data": data_hoje, "nivel": "INFO"}
        )
    ...
    def test_filtrar_logs_nivel_warning(self, admin_autenticado):
        ...
        response = admin_autenticado.post(
            "/admin/auditoria/filtrar", data={"data": data_hoje, "nivel": "WARNING"}
        )
    ...
    def test_filtrar_logs_nivel_error(self, admin_autenticado):
        ...
        response = admin_autenticado.post(
            "/admin/auditoria/filtrar", data={"data": data_hoje, "nivel": "ERROR"}
        )
    ...
    def test_filtrar_logs_data_sem_arquivo(self, admin_autenticado):
        ...
        response = admin_autenticado.post(
            "/admin/auditoria/filtrar", data={"data": "2000-01-01", "nivel": "TODOS"}
        )

        assert response.status_code == status.HTTP_200_OK
        # Deve ter mensagem sobre arquivo não encontrado
        assert (
            "encontrado" in response.text.lower() or "nenhum" in response.text.lower()
        )
    ...
    def test_aluno_nao_pode_filtrar_logs(self, aluno_autenticado):
        """Aluno não deve poder filtrar logs"""
        data_hoje = agora().strftime("%Y-%m-%d")
...
class TestSegurancaConfiguracoes:
    ...
    def test_professor_nao_acessa_tema(self, professor_autenticado):
        """Professor não deve acessar temas"""
        response = professor_autenticado.get("/admin/tema", follow_redirects=False)
        assert response.status_code in [
            status.HTTP_303_SEE_OTHER,
            status.HTTP_403_FORBIDDEN,
        ]

    def test_professor_nao_acessa_auditoria(self, professor_autenticado):
        """Professor não deve acessar auditoria"""
        response = professor_autenticado.get("/admin/auditoria", follow_redirects=False)
        assert response.status_code in [
            status.HTTP_303_SEE_OTHER,
            status.HTTP_403_FORBIDDEN,
        ]
    ...
    def test_listar_configuracoes_erro_banco(self, admin_autenticado):
        ...
        with patch("routes.admin_configuracoes_routes.configuracao_repo") as mock_repo:
            ...
            response = admin_autenticado.get(
                "/admin/configuracoes", follow_redirects=False
            )
...
class TestAplicarTemaErros:
    ...
    def test_aplicar_tema_rate_limit(self, admin_autenticado):
        ...
        with patch(
            "routes.admin_configuracoes_routes.admin_config_limiter.verificar",
            return_value=False,
        ):
            response = admin_autenticado.post(
                "/admin/tema/aplicar", data={"tema": "original"}, follow_redirects=False
    ...
    def test_aplicar_tema_erro_salvar_banco(self, admin_autenticado):
        ...
        if css_original.exists():
            with patch(
                "routes.admin_configuracoes_routes.configuracao_repo"
            ) as mock_repo:
...
class TestSalvarLoteConfiguracoes:
    ...
    async def test_salvar_lote_sucesso_com_avisos(self):
        ...
        request = self._criar_request_mock(
            {"toast_delay": "5000", "chave_inexistente": "valor"}
        )
    ...
    async def test_salvar_lote_validation_error(self):
        ...
        with patch(
            "routes.admin_configuracoes_routes.SalvarConfiguracaoLoteDTO"
        ) as mock_dto:
    ...
    async def test_salvar_lote_validation_error_erros_vazios(self):
        ...
        with patch(
            ...
            with patch(
                "routes.admin_configuracoes_routes.processar_erros_validacao",
                return_value={},
            ):
                response = await post_salvar_lote_configuracoes(request)
...
class TestLerLogArquivo:
    ...
    def test_arquivo_muito_grande(self, admin_autenticado):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "routes.admin_configuracoes_routes._ler_log_arquivo"
            ) as mock_ler:
                mock_ler.return_value = (
                    "",
                    0,
                    "Arquivo de log muito grande (11.00 MB)",
                )
    ...
    def test_erro_leitura_arquivo(self, admin_autenticado):
        ...
        with patch("routes.admin_configuracoes_routes._ler_log_arquivo") as mock_ler:
            mock_ler.return_value = (
                "",
                0,
                "Erro ao ler arquivo de log: Permission denied",
            )
