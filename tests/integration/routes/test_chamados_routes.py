...
class TestCriarChamado:
    ...
    def test_get_formulario_cadastro(self, aluno_autenticado):
        """Usuário autenticado deve acessar formulário de cadastro"""
        response = aluno_autenticado.get("/chamados/cadastrar")
        assert response.status_code == status.HTTP_200_OK
        assert "chamado" in response.text.lower()

    def test_criar_chamado_com_dados_validos(self, aluno_autenticado):
        """Deve criar chamado com dados válidos"""
        response = aluno_autenticado.post(
            "/chamados/cadastrar",
            data={
    ...
    def test_criar_chamado_titulo_curto(self, aluno_autenticado):
    ...
    def test_criar_chamado_titulo_longo(self, aluno_autenticado):
    ...
    def test_criar_chamado_descricao_curta(self, aluno_autenticado):
    ...
    def test_criar_chamado_sem_prioridade(self, aluno_autenticado):
    ...
    def test_criar_chamado_prioridade_invalida(self, aluno_autenticado):
    ...
    def test_chamado_criado_com_status_aberto(self, aluno_autenticado):
        """Chamado criado deve ter status 'Aberto'"""
        # Criar chamado
        aluno_autenticado.post(
            "/chamados/cadastrar",
            data={
        ...
        response = aluno_autenticado.get("/chamados/listar")
        assert response.status_code == status.HTTP_200_OK
...
class TestListarChamados:
    ...
    def test_listar_chamados_usuario(self, aluno_autenticado):
        """Usuário deve conseguir listar seus chamados"""
        response = aluno_autenticado.get("/chamados/listar")
        assert response.status_code == status.HTTP_200_OK
        assert "chamado" in response.text.lower()

    def test_usuario_ve_apenas_proprios_chamados(self, client, aluno_autenticado):
        """Usuário deve ver apenas seus próprios chamados"""
        # Este aluno já está autenticado como um usuário comum
...
class TestVisualizarChamado:
    ...
    def test_visualizar_proprio_chamado(self, aluno_autenticado):
        ...
        response = aluno_autenticado.get("/chamados/1/visualizar")
        assert response.status_code == status.HTTP_200_OK
        assert "Problema urgente" in response.text

    def test_visualizar_chamado_inexistente(self, aluno_autenticado):
        """Deve retornar erro ao visualizar chamado inexistente"""
        response = aluno_autenticado.get(
            "/chamados/999/visualizar", follow_redirects=False
        )
...
class TestExcluirChamado:
    ...
    def test_excluir_proprio_chamado(self, aluno_autenticado):
        ...
        # Excluir
        response = aluno_autenticado.post("/chamados/1/excluir", follow_redirects=False)
        assert response.status_code == status.HTTP_303_SEE_OTHER

    def test_excluir_chamado_inexistente(self, aluno_autenticado):
        """Deve retornar erro ao excluir chamado inexistente"""
        response = aluno_autenticado.post(
            "/chamados/999/excluir", follow_redirects=False
        )
        assert response.status_code == status.HTTP_303_SEE_OTHER
...
class TestAdminListarChamados:
    ...
    def test_admin_lista_todos_chamados(
        self, client, admin_teste, criar_usuario, fazer_login
    ):
    ...
    def test_usuario_comum_nao_acessa_admin_listagem(self, aluno_autenticado):
...
class TestAdminResponderChamado:
    ...
    def test_admin_acessa_formulario_responder(
        self, aluno_autenticado, admin_autenticado
    ):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert (
            "resposta" in response.text.lower() or "responder" in response.text.lower()
        )

    def test_admin_responde_chamado_com_sucesso(
        self, client, admin_teste, criar_usuario, fazer_login
    ):
    ...
    def test_admin_muda_status_para_em_analise(
        self, aluno_autenticado, admin_autenticado
    ):
    ...
    def test_admin_muda_status_para_resolvido(
        self, aluno_autenticado, admin_autenticado
    ):
    ...
    def test_resposta_curta_e_rejeitada(
        self, client, admin_teste, criar_usuario, fazer_login
    ):
        ...
        response = client.post(
            "/admin/chamados/1/responder",
            data={"mensagem": "OK", "status_chamado": "Resolvido"},  # Muito curto
            follow_redirects=True,
        )
    ...
    def test_usuario_comum_nao_pode_responder(self, aluno_autenticado):
...
class TestHistoricoInteracoes:
    ...
    def test_interacao_registra_admin(
        self, client, admin_teste, criar_usuario, fazer_login
    ):
    ...
    def test_multiplas_respostas_do_mesmo_admin(
        self, client, admin_teste, criar_usuario, fazer_login
    ):
    ...
    def test_usuario_pode_responder_proprio_chamado(self, aluno_autenticado):
        ...
        response = aluno_autenticado.post(
            "/chamados/1/responder",
            data={"mensagem": "Informação adicional sobre o problema reportado"},
            follow_redirects=False,
        )
    ...
    def test_historico_mostra_todas_interacoes(
        self, aluno_autenticado, admin_autenticado
    ):
        ...
        response = aluno_autenticado.post(
            "/chamados/1/responder",
            data={"mensagem": "Resposta do usuário com mais informações"},
            follow_redirects=False,
        )
        assert response.status_code == status.HTTP_303_SEE_OTHER
...
class TestFluxoCompleto:
    ...
    def test_fluxo_completo_usuario_e_admin(self, aluno_autenticado, admin_autenticado):
...
class TestReabrirChamado:
    ...
    def test_admin_reabre_chamado_fechado(self, aluno_autenticado, admin_autenticado):
    ...
    def test_reabrir_chamado_nao_fechado(self, aluno_autenticado, admin_autenticado):
    ...
    def test_reabrir_chamado_inexistente(self, admin_autenticado):
        ...
        response = admin_autenticado.post(
            "/admin/chamados/999/reabrir", follow_redirects=False
        )
        assert response.status_code == status.HTTP_303_SEE_OTHER
...
class TestErrosAdminChamados:
    ...
    def test_erro_ao_salvar_resposta(self, aluno_autenticado, admin_autenticado):
    ...
    def test_erro_ao_fechar_chamado(self, aluno_autenticado, admin_autenticado):
    ...
    def test_erro_ao_reabrir_chamado(self, aluno_autenticado, admin_autenticado):
...
class TestRateLimitChamados:
    ...
    def test_rate_limit_criar_chamado(self, aluno_autenticado):
        """Rate limit deve bloquear criação excessiva de chamados"""
        from unittest.mock import patch

        with patch(
            "routes.chamados_routes.chamado_criar_limiter.verificar", return_value=False
        ):
            response = aluno_autenticado.post(
                "/chamados/cadastrar",
                data={
            ...
            assert response.status_code == status.HTTP_200_OK
            assert (
                "muitas tentativas" in response.text.lower()
                or "aguarde" in response.text.lower()
            )

    def test_rate_limit_responder_chamado(self, aluno_autenticado):
        ...
        with patch(
            "routes.chamados_routes.chamado_responder_limiter.verificar",
            return_value=False,
        ):
            response = aluno_autenticado.post(
                "/chamados/1/responder",
                data={"mensagem": "Mensagem de resposta no chamado"},
                follow_redirects=False,
            )
...
class TestPermissoesChamados:
    ...
    def test_visualizar_chamado_de_outro_usuario(
        self, client, criar_usuario, fazer_login
    ):
    ...
    def test_responder_chamado_de_outro_usuario(
        self, client, criar_usuario, fazer_login
    ):
        ...
        response = client.post(
            "/chamados/1/responder",
            data={"mensagem": "Tentativa de resposta indevida no chamado"},
            follow_redirects=False,
        )
        assert response.status_code == status.HTTP_303_SEE_OTHER
...
class TestExclusaoChamado:
    ...
    def test_excluir_chamado_nao_aberto(self, aluno_autenticado, admin_autenticado):
    ...
    def test_excluir_chamado_com_resposta_admin(
        self, aluno_autenticado, admin_autenticado
    ):
...
class TestValidacaoResposta:
    ...
    def test_resposta_invalida_validation_error(self, aluno_autenticado):
        ...
        response = aluno_autenticado.post(
            "/chamados/1/responder", data={"mensagem": "abc"}  # Muito curta
        )

        assert response.status_code == status.HTTP_200_OK

    def test_responder_chamado_inexistente(self, aluno_autenticado):
        """Deve redirecionar quando chamado não existe"""
        response = aluno_autenticado.post(
            "/chamados/9999/responder",
            data={"mensagem": "Tentativa de resposta em chamado inexistente"},
            follow_redirects=False,
        )
