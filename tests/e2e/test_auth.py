...
class TestCadastroAcessoPagina:
    ...
    def test_pagina_cadastro_possui_opcoes_perfil(
        ...
        """Deve exibir opcoes de perfil (Aluno/Professor)."""
        page = CadastroPage(e2e_page, e2e_server)
        page.navegar()

        expect(e2e_page.locator('label[for="perfil_Aluno"]')).to_be_visible()
        expect(e2e_page.locator('label[for="perfil_Professor"]')).to_be_visible()
...
class TestCadastroSucesso:
    ...
    def test_cadastro_aluno_com_dados_validos(
        self, e2e_page: Page, e2e_server: str, limpar_banco_e2e
    ):
        """Deve cadastrar usuario Aluno com dados validos."""
        page = CadastroPage(e2e_page, e2e_server)
        page.navegar()
    ...
    def test_cadastro_professor_com_dados_validos(
        self, e2e_page: Page, e2e_server: str, limpar_banco_e2e
    ):
        """Deve cadastrar usuario Professor com dados validos."""
        page = CadastroPage(e2e_page, e2e_server)
        page.navegar()
...
class TestCadastroValidacaoPerfil:
    ...
    def test_perfil_aluno_selecionado_corretamente(
        self, e2e_page: Page, e2e_server: str, limpar_banco_e2e
    ):
        """Deve permitir selecionar perfil Aluno."""
        page = CadastroPage(e2e_page, e2e_server)
        page.navegar()
    ...
    def test_perfil_professor_selecionado_corretamente(
        ...
        """Deve permitir selecionar perfil Professor."""
        page = CadastroPage(e2e_page, e2e_server)
        page.navegar()
...
class TestLoginSucesso:
    ...
    def test_login_professor_com_credenciais_validas(
        self, e2e_page: Page, e2e_server: str, limpar_banco_e2e
    ):
        """Deve fazer login com credenciais validas de professor."""
        email = "professor_login@example.com"
        senha = "SenhaForte@123"
