...
class TestRotasPublicasComUsuarioLogado:
    ...
    def test_home_acessivel_usuario_logado(self, aluno_autenticado):
        """Usuário logado também pode acessar landing page"""
        response = aluno_autenticado.get("/")
        assert response.status_code == status.HTTP_200_OK

    def test_sobre_acessivel_usuario_logado(self, aluno_autenticado):
        """Usuário logado pode acessar página sobre"""
        response = aluno_autenticado.get("/sobre")
        assert response.status_code == status.HTTP_200_OK
