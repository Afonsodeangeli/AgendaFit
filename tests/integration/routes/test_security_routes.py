...
class TestSQLInjection:
    ...
    def test_login_com_sql_injection_no_email(
        ...
        response = client.post(
            "/login",
            data={"email": sql_injection_email, "senha": "qualquercoisa"},
        ...
        assert (
            "e-mail ou senha" in response.text.lower()
            or "inválido" in response.text.lower()
        )
    ...
    def test_buscar_usuario_por_id_com_sql_injection(self, admin_autenticado):
        ...
        response = admin_autenticado.get(
            f"/admin/usuarios/editar/{malicious_id}", follow_redirects=False
        )
...
class TestEscalacaoPrivilegios:
    ...
    def test_aluno_nao_pode_acessar_admin_usuarios(self, aluno_autenticado):
        """Aluno não deve acessar painel de administração de usuários"""
    ...
    def test_aluno_nao_pode_criar_usuario_como_admin(self, aluno_autenticado):
        """Aluno não deve poder acessar endpoint de criação de usuário admin"""
    ...
    def test_professor_nao_pode_acessar_backups(self, professor_autenticado):
        """Professor não deve acessar área de backups (apenas admin)"""
        response = professor_autenticado.get(
    ...
    def test_professor_nao_pode_criar_backup(self, professor_autenticado):
        """Professor não deve poder criar backups"""
        response = professor_autenticado.post(
    ...
    def test_usuario_nao_pode_editar_outro_usuario(
        self, client, dois_usuarios, fazer_login
    ):
...
class TestRateLimiting:
    ...
    def test_multiplas_tentativas_login_falhas_sao_limitadas(self, client):
        ...
        for i in range(6):
            response = client.post(
                "/login",
                data={"email": "inexistente@example.com", "senha": "SenhaErrada@123"},
                follow_redirects=True,
            )
        ...
        assert (
            ...
            or "bloqueado" in response.text.lower()
        )
    ...
    def test_cadastro_limitado_por_ip(self, client):
        ...
        assert (
            "muitas tentativas" in response.text.lower()
            or "limite" in response.text.lower()
            or response.status_code == 429
        )  # Too Many Requests

    def test_esqueci_senha_fortemente_limitado(
        self, client, criar_usuario, usuario_teste
    ):
        ...
        response1 = client.post(
            "/esqueci-senha",
            data={"email": usuario_teste["email"]},
            follow_redirects=False,
        )
        ...
        assert (
            "aguarde" in response2.text.lower()
            or "muitas tentativas" in response2.text.lower()
        )
...
class TestAutorizacaoArquivos:
    ...
    def test_usuario_nao_pode_baixar_backup_diretamente(
        self, aluno_autenticado, criar_backup
    ):
        """Usuário comum não deve poder baixar backups"""

        # Criar backup como admin (precisamos mockar ou usar admin)
        # Este teste assume que aluno_autenticado não é admin

        # Tentar baixar backup (mesmo sem saber o nome)
        response = aluno_autenticado.get(
            "/admin/backups/download/algum_backup.db", follow_redirects=False
        )
...
class TestSessionSecurity:
    ...
    def test_logout_invalida_sessao(self, aluno_autenticado):
        """Logout deve invalidar sessão e prevenir acesso subsequente"""

        # Verificar que está logado antes do logout
        response_antes = aluno_autenticado.get("/usuario", follow_redirects=False)
        assert (
            response_antes.status_code == 200
        ), "Deveria estar autenticado antes do logout"

        # Fazer logout
        response_logout = aluno_autenticado.get("/logout", follow_redirects=False)
        assert_redirects_to(response_logout, "/")

        # Tentar acessar área protegida com mesma sessão
        response_protegido = aluno_autenticado.get("/usuario", follow_redirects=False)
    ...
    def test_sessao_nao_compartilhada_entre_usuarios(self, client, criar_usuario):
        ...
        # Login como usuario1
        client.post("/login", data={"email": "user1@example.com", "senha": "Senha@123"})
        ...
        # Login como usuario2
        client.post("/login", data={"email": "user2@example.com", "senha": "Senha@123"})
...
class TestPasswordSecurity:
    ...
    def test_senha_nao_retornada_em_respostas(
        self, client, criar_usuario, usuario_teste
    ):
        ...
        client.post(
            "/login",
            data={"email": usuario_teste["email"], "senha": usuario_teste["senha"]},
        )
...
class TestInformationDisclosure:
    ...
    def test_usuario_inexistente_nao_revela_existencia(self, client):
        ...
        response = client.post(
            "/login",
            data={"email": "naoexiste@example.com", "senha": "Senha@123"},
            follow_redirects=True,
        )
