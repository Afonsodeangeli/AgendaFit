...
class TestListarBackups:
    ...
    def test_listar_backups_requer_admin(self, aluno_autenticado):
        """Aluno não deve acessar listagem de backups"""
    ...
    def test_professor_nao_acessa_listagem(self, professor_autenticado):
        """Professor não deve acessar listagem de backups"""
...
class TestCriarBackup:
    ...
    def test_aluno_nao_pode_criar_backup(self, aluno_autenticado):
        """Aluno não deve poder criar backup"""
    ...
    def test_professor_nao_pode_criar_backup(self, professor_autenticado):
        """Professor não deve poder criar backup"""
...
class TestRestaurarBackup:
    ...
    def test_aluno_nao_pode_restaurar_backup(self, aluno_autenticado, criar_backup):
        """Aluno não deve poder restaurar backup"""
        criar_backup()
        from util import backup_util
...
class TestExcluirBackup:
    ...
    def test_aluno_nao_pode_excluir_backup(self, aluno_autenticado, criar_backup):
        """Aluno não deve poder excluir backup"""
        criar_backup()
        from util import backup_util
...
class TestDownloadBackup:
    ...
    def test_aluno_nao_pode_baixar_backup(self, aluno_autenticado, criar_backup):
        """Aluno não deve poder baixar backup"""
        criar_backup()
        from util import backup_util
    ...
    def test_professor_nao_pode_baixar_backup(
        self, professor_autenticado, criar_backup
    ):
        """Professor não deve poder baixar backup"""
        criar_backup()
        from util import backup_util
...
class TestRateLimitBackups:
    ...
    def test_rate_limit_criar_backup(self, admin_autenticado):
        ...
        with patch(
            "routes.admin_backups_routes.admin_backups_limiter.verificar",
            return_value=False,
        ):
            response = admin_autenticado.post(
                "/admin/backups/criar", follow_redirects=True
            )

            assert response.status_code == status.HTTP_200_OK
            assert (
                "muitas" in response.text.lower() or "aguarde" in response.text.lower()
            )
    ...
    def test_rate_limit_restaurar_backup(self, admin_autenticado, criar_backup):
        ...
        with patch(
            ...
            response = admin_autenticado.post(
                f"/admin/backups/restaurar/{nome_backup}", follow_redirects=True
            )
    ...
    def test_rate_limit_excluir_backup(self, admin_autenticado, criar_backup):
        ...
        with patch(
            ...
            response = admin_autenticado.post(
                f"/admin/backups/excluir/{nome_backup}", follow_redirects=True
            )
    ...
    def test_rate_limit_download_backup(self, admin_autenticado, criar_backup):
        ...
        with patch(
            "routes.admin_backups_routes.backup_download_limiter.verificar",
            return_value=False,
        ):
            response = admin_autenticado.get(
                f"/admin/backups/download/{nome_backup}", follow_redirects=True
            )
...
class TestErroCriarBackup:
    ...
    def test_erro_criar_backup(self, admin_autenticado):
        ...
        with patch(
            "routes.admin_backups_routes.backup_util.criar_backup",
            return_value=(False, "Erro ao criar backup"),
        ):
            response = admin_autenticado.post(
                "/admin/backups/criar", follow_redirects=True
            )
